...
- **Public signup** — disabled by default (`PUBLIC_SIGNUP_ENABLED` opt-in): [signup-hardening-plan.md](./driving_school_platform/nextjs_space/docs/engineering/signup-hardening-plan.md). **Invite-only (B2B)** — copy-link mode on Admin Users + accept page: [invite-only-foundation-plan.md](./driving_school_platform/nextjs_space/docs/engineering/invite-only-foundation-plan.md). **Transactional email** — evaluation only (no send yet): [email-provider-evaluation.md](./driving_school_platform/nextjs_space/docs/engineering/email-provider-evaluation.md).
