...
| **Node.js runtime** | Migration to **Node 24** is active in `node-24-runtime-migration-v1`: local compatibility passed on Node 24.18.0; the GitLab branch pipeline passed using `node:24`; the Vercel Preview for `e5ddc39` deployed successfully with Node 24.x. The Vercel Project Setting is aligned to Node 24.x; main merge, Production deployment, and post-deploy hosted verification remain pending. |
