...
| **Git / CI** | Published implementation `8d575aa166bc906e389c5f093343b45906933f34` (`fix(billing): make event projection atomic and idempotent`) is on local `main` and `origin/main` at the hosted-validation checkpoint. A later docs-only canonical-closure commit may advance HEAD without changing already-proven behavioral evidence — do not invent that SHA. Solution #5 source branch cleanup **COMPLETE** (local and remote). Solution #6 source branch **ABSENT** local + remote (**CLOSED AND CLEANED UP**). Solution #7 source branch `billing-projection-atomic-idempotency-v1` **retained** at the same implementation SHA pending closure-commit hosted validation. Node 24 runtime application merge `909b69a` and closure record `a397054` remain integrated; GitLab `main` pipeline baseline used Docker image `node:24`. Re-resolve live refs through Git — not eternal SHAs. |
...
| **Solution #7** | `billing-projection-atomic-idempotency-v1` / `BILLING-SEC-002` — **PUBLISHED ON MAIN** / **HOSTED MAIN VALIDATION PASS** / **CLOSED**. Finding `BILLING-SEC-002` **CLOSED**. Durable implementation anchor `8d575aa166bc906e389c5f093343b45906933f34`. Hosted main pipeline `2813791386` / IID **385** **SUCCESS**; jobs `check` **16261884876** + `database-integration` **16261884877** **SUCCESS** (`allow_failure: false`). Durable technical conclusions: billing event projection and lifecycle completion are one PostgreSQL transaction; BillingEvent row serialization uses `FOR UPDATE`; replay of `PROCESSED` is idempotent; failed transactional apply rolls back commercial state; conditional `FAILED` recording cannot overwrite `PROCESSED`; persisted/payload organization mismatch cannot cross tenant boundary; webhook remains fail-closed under `BILLING-SEC-001`; no schema/migration was required; real PostgreSQL integration proves rollback/retry/concurrency; `TEST-GATE-001` remains separate. Source branch `billing-projection-atomic-idempotency-v1` **retained** pending closure-commit hosted validation. Cleanup only after: closure commit → published main → closure pipeline SUCCESS. |
| **Ordered next** | `jwt-session-revocation-v1` / `AUTH-SESSION-001` — **NOT STARTED**. Do not begin. Queued unchanged: `LICENSING-002`, `LICENSING-001`, `AUTH-RATE-001`, `TEST-GATE-001`. |
