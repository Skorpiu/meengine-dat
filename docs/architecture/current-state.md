...
| **Git / CI** | Solution #8 implementation anchor `5af60d259d9f394590f926ceaaacf6f454aac282` (`fix(auth): add server-verifiable JWT session revocation`) is integrated on `main`. Main technical pipeline `2819681959` / IID **389** **SUCCESS**. Closure record = current docs-only closure publication; resolve live `main`. Durable Solution #7 implementation anchor `8d575aa166bc906e389c5f093343b45906933f34` remains integrated. Solutions #5–#7 source branches **ABSENT** / cleaned up. Solution #8 source branch `jwt-session-revocation-v1` **STILL PRESENT** at `5af60d259d9f394590f926ceaaacf6f454aac282` (**SOURCE CLEANUP PENDING**). Node 24 runtime application merge `909b69a` and closure record `a397054` remain integrated; GitLab `main` pipeline baseline used Docker image `node:24`. Re-resolve live refs through Git — not eternal SHAs. |
...
| **Solution #8** | `jwt-session-revocation-v1` / `AUTH-SESSION-001` — **CLOSED ON PUBLISHED MAIN** / **SOURCE CLEANUP PENDING**. Finding `AUTH-SESSION-001` **CLOSED**. Architecture `USER_SESSION_VERSION_EPOCH` (`User.authSessionVersion` Int `@default(0)` + JWT claim `authSessionVersion`; legacy missing claim = 0; fail closed on mismatch / missing user / DB verification error). Framework authority: NextAuth **4.24.15**. Implementation anchor `5af60d259d9f394590f926ceaaacf6f454aac282`. Activation `a47493ddac65dc74c102f9f52a2b38d38ad24b3e`. Migration `20260903120000_add_user_auth_session_version` **APPLIED** on Production by **human** operator via DEC-069 (`AUTH_SESSION_REMOTE_SCHEMA_READY=yes`; schema up to date). Hosted source pipeline `2816551646` / IID **388** SUCCESS. Main technical pipeline `2819681959` / IID **389** SUCCESS. Production Ready + read-only smoke **PASS** (see Hosted smoke). Same migrate deploy also applied historical `20260714160000_platform_commercial_catalog_schema_foundation_v1` (human 2026-09-03; technical risk LOW; governance deviation yes; no commercial seed; no rollback). Queued unnumbered follow-ups: `migration-deploy-pending-set-enumeration-v1`; `production-smoke-hosted-timeout-hardening-v1` (not remediated in Solution #8). Source branch **STILL PRESENT**. Closure record = current docs-only closure publication; resolve live `main`. |
| **Ordered next** | Solution #8 source-branch cleanup **PENDING AUTHORIZATION**. Ordered technical next after cleanup: `LICENSING-002`, then `LICENSING-001`, `AUTH-RATE-001`, `TEST-GATE-001`. Do **not** begin LICENSING while cleanup is pending. |
...
- `AUTH-SESSION-001` is **CLOSED** by Solution #8 / `jwt-session-revocation-v1` (**CLOSED ON PUBLISHED MAIN** / **SOURCE CLEANUP PENDING**; implementation anchor `5af60d259d9f394590f926ceaaacf6f454aac282`). Historical Wave A3 wording that the finding “remains open” is superseded.
