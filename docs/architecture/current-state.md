...
| **Solution #5** | `database-integration-test-harness-v1` / `TEST-ARCH-001` / DEC-070 — **LOCAL IMPLEMENTATION + INDEPENDENT EQR COMPLETE**. Branch `database-integration-test-harness-v1` exists **LOCAL ONLY**; source branch **not yet published**; implementation **uncommitted**. Baseline HEAD `271a9837c48e3cee2f8d6c3a9c27e2c233f4eb3b`; baseline tree `aa4b89544cffba49fbf14db4503616b8c648a68a`. Reviewed EQR snapshot SHA256 `b5c6187f2db5f5abdd578eed8894bbd2c5217849505c45e3a17435fbd5d554aa`. Local EQR **PASS**; two consecutive fresh disposable integration runs **PASS** (29/29 exact committed migrations from zero; 4/4 real PostgreSQL proofs; teardown each run). Canonical local `pnpm check` **PASS**: 216/216 files; 1843/1843 tests; 0 lint errors / 51 known warnings; typecheck PASS; Next.js 16.3.1 build PASS. GitLab `database-integration` job implemented in working copy; **HOSTED VALIDATION PENDING** source publication. **Not** CLOSED ON PUBLISHED MAIN. Capability boundary: disposable `dat_it` bootstrap runs with PostgreSQL superuser privilege — valid for migration history, FK, rollback, and lock proofs; **not** application-role / RLS / least-privilege evidence. Runbook: [database-integration-tests.md](../../driving_school_platform/nextjs_space/docs/ops/database-integration-tests.md). |
| **Ordered next** | Solution #5 — **LOCAL PRE-COMMIT REVIEW / COMMIT AUTHORIZATION** (not source publication). After commit authorization: source publication → hosted source validation → main integration → main publication → hosted main validation → closure/cleanup as required. Solution #6 `e2e-suite-contract-repair-v1` / `TEST-HYGIENE-001` remains **NOT STARTED** until Solution #5 publication lifecycle completes. `platform-separation-architecture-plan-v1` remains a parallel architecture lane. `dependency-security-monitoring-v1` and `TOOLCHAIN-002` remain separate. Do not run ordinary local `pnpm dev` until local `.env` / `.env.local` database URLs are loopback (current hosted `.env` is expected to fail `env:check`). |
