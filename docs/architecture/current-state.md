...
- `tenant-operational-organization-id-not-null-migrations-v1` — migration `20260610140000_make_operational_organization_id_required`: `SET NOT NULL` on `students`, `instructors`, `vehicles`, `lessons`, `exams`, `lesson_requests`; Prisma schema `organizationId String` on six operational models; operator scripts use raw SQL for legacy NULL probes (`lib/tenant-operational-organization-id-sql.ts`). **No `migrate deploy` run in batch.** Human deploy + smoke QA pending per environment. Validated via `pnpm check`.

### Likely next (smallest safe slices)

1. **Operator:** human `migrate deploy` + post-migration smoke tests on target DB (see readiness doc) — not a code batch
