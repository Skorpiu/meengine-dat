...
- `tenant-operational-organization-id-not-null-migrations-plan-v1` — docs-only D4 gate: GO/NO-GO checklist, operator pre-migration battery (`.env` + `.env.local`, `DATABASE_URL`, `DIRECT_URL`, migrate status, null report, backfill dry-run), single-migration proposal, post-migration smoke tests; report [tenant-operational-organization-id-not-null-readiness.md](./tenant-operational-organization-id-not-null-readiness.md). Scripts confirmed: `pnpm tenant:org-null-report`, `pnpm tenant:org-backfill:dry-run`. No schema/migration/runtime/data changes. Validated via `pnpm check`.

### Likely next (smallest safe slices)

1. `tenant-operational-organization-id-not-null-migrations-v1` — **deferred (D4):** `SET NOT NULL` on six operational tables; requires operator GO on gate + `APPROVED TO IMPLEMENT: tenant-operational-organization-id-not-null-migrations-v1` + human `migrate deploy`; re-run gate battery on target DB immediately before deploy
