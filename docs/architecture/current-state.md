...
- `dat-production-smoke-canonical-fixtures-v1` — **DEC-064:** canonical fixture names + smoke feature overrides + reconcile CLI (`pnpm ops:reconcile-production-smoke-fixtures`, dry-run default / `--apply`); seed local fidelity; inspector canonical readiness. **Remote `--apply` not executed by agent.**

### Likely next (commercial path)

1. **Human dry-run then `--apply`** for `pnpm ops:reconcile-production-smoke-fixtures` on the validated smoke target (after target env vars) — then vault fixture capture + hosted smoke re-run
