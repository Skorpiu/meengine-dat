...
| **GitLab CI isolation** | `gitlab-ci-database-isolation-v1` — **IMPLEMENTED + VALIDATED IN-BRANCH**; **NOT YET INTEGRATED ON MAIN**. Base `14c3865372502f074941a1fc81a55b5ec7f1b589`. Remediation: GitLab `before_script` reasserts process-local loopback `DATABASE_URL` / `DIRECT_URL` before `pnpm install` / `pnpm check`. Higher-precedence GitLab CI/CD variables can override YAML defaults; runner-provided environment may also supply values to the job shell. The `before_script` shell export deterministically reasserts the loopback URLs after environment injection. DEC-068 unchanged (no CI remote-DB escape). Zero DB / zero migration. GitLab hosted pipeline **PENDING** external proof — do not claim pipeline green from local evidence. Cause classification: repository evidence supports external job-environment injection as the cause class. Higher-precedence GitLab CI/CD variables (for example policy/pipeline/project/group/instance variables) are the most likely source. Runner-provided environment remains another externally confirmable possibility. |
| **Solution #4** | `migration-deploy-target-safety-gate-v1` / `DB-MIGRATION-001` remains published at `origin/migration-deploy-target-safety-gate-v1` @ `ec39e05d2ffda40f439f3bdadc7889853e5ca9ef`. **Not on this branch. Do not modify it.** Merge remains blocked until `gitlab-ci-database-isolation-v1` reaches a green GitLab pipeline and is integrated into `main`. |
| **Ordered next** | Finish `gitlab-ci-database-isolation-v1` (GitLab pipeline still PENDING). Then Solution #4 merge remains blocked until this CI slice is on `main`. Then `platform-separation-architecture-plan-v1` as a parallel architecture lane → `dependency-security-monitoring-v1` remains separate; then P1/P2 according to the post-audit queue. Do not run ordinary local `pnpm dev` until the local `.env` / `.env.local` database URLs are loopback (current hosted `.env` is expected to fail `env:check`). |
...
- Active integration-blocker slice: `gitlab-ci-database-isolation-v1` —
  **IMPLEMENTED + VALIDATED IN-BRANCH**; **NOT YET INTEGRATED ON MAIN**.
  ...
  install/check. DEC-068 unchanged. Zero DB / zero migration. GitLab hosted
  pipeline **PENDING** external proof.
