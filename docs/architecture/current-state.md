...
- `dat-production-smoke-canonical-fixtures-v1` — **DEC-064 (corrected):** manual rename targets (`Smoke Instructor 1`/`Smoke Student 1` + negatives); invite fixtures (`Smoke Instructor 2`/`Smoke Student 2`) require real ACCEPTED invitations matched by operator-only exact emails — absence is a **blocker**; Sarah Williams/`INS-002` and Bob Wilson/`STU-002` preserved as additional (never renamed to invite-canonical names); smoke feature overrides + plates; reconcile CLI dry-run/`--apply`. **Remote dry-run found invite blockers; `--apply` not recommended until invites are accepted via the app. Agent did not execute `--apply`.**

### Likely next (commercial path)

1. **Human:** set `DAT_SMOKE_INVITED_INSTRUCTOR_EMAIL` / `DAT_SMOKE_INVITED_STUDENT_EMAIL` in `.env.operator.production.local`; send+accept real invites in the app; re-run operator-safe dry-run; only then `--apply` for names/features/plates — then vault fixture capture + hosted smoke
2. **`platform-separation-architecture-plan-v1`** — recommended next after smoke fixtures close (autonomous MeEngine Platform vs DAT)
3. **`platform-commercial-catalog-read-services-v1`** — deferred until Platform separation plan gates commercial work (do not resume blindly inside embedded `/platform`)
