...
- `instructor-delete-policy-v1-docs` — Docs-only formal policy for Instructor **Delete** (zero-deps hard delete), **Deactivate** (normal leave-school path), and deferred **Remove app access**; eligibility matrix; future API contracts; legacy `DELETE /api/users/delete` guard requirement. Policy: [instructor-delete-policy.md](./instructor-delete-policy.md). DEC-019. **Runtime:** Profiles Delete remains UI-blocked; App Accounts legacy delete unchanged. Validated via `pnpm check`.

### Likely next (smallest safe slices)

1. `instructor-hard-delete-zero-deps-v1` — `DELETE /api/admin/instructors/[id]` + Profiles Delete allowed/blocked UI + guard on `DELETE /api/users/delete` for INSTRUCTOR (D4 / sensitive)
2. `instructor-deactivate-v1` — deactivate endpoint + UI (normal path when instructor leaves; preserve history)
3. `people-management-app-accounts-demote-v1` — demote App accounts tab **after** policy-driven instructor delete/deactivate + legacy guard
