...
DAT has a **working audit write boundary** (`lib/audit/audit-log-service.ts` + domain helpers) and a **tenant-aware `audit_logs` schema** (migration `20260702120000_audit_log_tenant_context_v1`). Twelve **distinct actions** are instrumented across **twelve admin routes**, covering the highest-traffic People + Lessons flows introduced during the first B2B cutline.

**Coverage posture (admin write paths):**

| Bucket | Approx. count | Notes |
| ------ | ------------- | ----- |
| **AUDITED** | 12 routes / 12 actions | See §2 |
| **COVERED_BY_OTHER_EVENT** | 1 | `POST /api/admin/students/[id]/invite` → `student.invite` (not `invitation.create`) |
| **CANDIDATE (P1)** | 8 | Lesson delete, invitation/instructor email change, instructor delete/reactivate, student create, manual practical history |
...
**Readiness verdict:** Safe to continue **small write-path slices** before building a viewer/read API. The next highest-value gap is **`lesson.delete`** (explicit MVP in DEC-044 foundation). After P1 lesson + invitation + instructor lifecycle gaps close, shift priority to **read API / operator viewer** unless import volume auditing becomes an operator requirement.
...
| `POST /api/admin/invitations/[id]/change-email` | Pending invite email update | **CANDIDATE** | `invitation.email.change` | Medium — token regen | **P1** | Flags only; no old/new email |
...
| -------- | -------- | ------ | ------ | ---- | -------- | --------------- |
| `POST /api/admin/students` | Manual student create (Onboarding) | **CANDIDATE** | `student.create` | Low | **P1** | Field names / `appAccessMode` only |
...
| `POST .../practical-lessons` | Manual completed practical history | **CANDIDATE** | `lesson.create` (source `DRIVING COMPLETED MANUAL`) | Low | **P1** | Reuse lesson helper; `lessonSource` in metadata |
...
| `DELETE /api/admin/lessons/[id]` | Delete lesson | **CANDIDATE** | `lesson.delete` | Medium — destructive | **P0/P1** | Snapshot ids/type before delete; no free text |
...
1. **`lesson.delete`** — only missing piece of the Lessons MVP triad (`create` / `update` / `delete`). Destructive; instructors can also DELETE via same route.
2. **`invitation.email.change`** — invitation MVP lists create/revoke/**change email**; route exists, token regeneration occurs; no audit today.
3. **Instructor lifecycle gaps** — `instructor.reactivate`, `instructor.delete`, `instructor.email.change` are active People flows without audit; symmetric to completed student slices.
4. **`student.create`** — manual Onboarding create; first touch of student record; low metadata risk.
5. **Manual practical history** (`POST .../practical-lessons`) — creates `DRIVING COMPLETED MANUAL` lessons; can reuse `lesson.create` with `lessonSource` metadata rather than a new action.
...
| - | ---------- | ----- | ------- |
| 1 | `audit-log-write-paths-lesson-delete-v1` | `DELETE /api/admin/lessons/[id]` → `lesson.delete` | Closes Lessons MVP per DEC-044; single route; helper extension in `lesson-audit.ts` |
| 2 | `audit-log-write-paths-invitation-email-change-v1` | `POST /api/admin/invitations/[id]/change-email` → `invitation.email.change` | Completes invitation admin triad; extend `invitation-audit.ts`; watch linked-student sync flags |
| 3 | `audit-log-write-paths-instructor-reactivate-v1` | `POST /api/admin/instructors/[id]/reactivate` → `instructor.reactivate` | Smallest instructor lifecycle gap; symmetric to existing `instructor.deactivate` |

**Defer to slice 4+ (not in top 3):** `instructor.delete`, `instructor.email.change`, `student.create`, manual practical-lesson POST (as `lesson.create`), import apply summaries.
...
| Manual practical lesson vs calendar lesson create | Prefer **`lesson.create`** + `lessonSource: DRIVING COMPLETED MANUAL` |
