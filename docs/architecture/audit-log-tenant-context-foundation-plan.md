...
**Status:** Foundation implemented in `audit-log-write-paths-foundation-v1`; invitation create/revoke wired in `audit-log-write-paths-integration-v1`; instructor qualified categories + deactivate wired in `audit-log-write-paths-people-v1`; lesson create + update wired in `audit-log-write-paths-lessons-v1`; student app-access remove + reactivate wired in `audit-log-write-paths-students-v1`; student profile PATCH + change-email wired in `audit-log-write-paths-student-profile-v1`; student hard-delete wired in `audit-log-write-paths-student-delete-v1`; student profile invite wired in `audit-log-write-paths-student-invite-v1` (`student.invite`, not duplicate `invitation.create`). Coverage readiness review: [audit-log-coverage-readiness-review.md](./audit-log-coverage-readiness-review.md) (`audit-log-coverage-readiness-review-v1`).
