...
- **Done (D4 migration file):** `tenant-operational-organization-id-not-null-migrations-v1` — migration `20260610140000_make_operational_organization_id_required`; schema NOT NULL on six operational models. Human `migrate deploy` + smoke QA pending per environment. Validated via `pnpm check`.
...
- **Next (operator):** human `migrate deploy` for `20260610140000_make_operational_organization_id_required` + smoke QA per environment. **Next gated code slice (recommended):** `supabase-rls-class-b-hardening-v1b` or other P1/P2 per priority. **Not recommended next:** `people-management-ux-unification-instructor-route-split-v1`.
...
- **Done (D4 migration file):** `tenant-operational-organization-id-not-null-migrations-v1` — `20260610140000_make_operational_organization_id_required`; human deploy pending. Route split **deferred (D4)**.
