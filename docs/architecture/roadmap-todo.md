...
- **Done (docs):** `invitation-email-update-policy-doc-v1` — [invitation-email-update-policy.md](./invitation-email-update-policy.md); **DEC-029**. Future runtime slices: `invitation-email-update-unlinked-instructor-v1`, etc.
- **Next gated slice (recommended):** `invitation-email-update-unlinked-instructor-v1` (runtime) or `tenant-operational-organization-id-not-null-migrations` (D4) per priority.
