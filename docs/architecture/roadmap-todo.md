...
- **Next gated slice (recommended):** `tenant-operational-organization-id-not-null-migrations-v1` (D4) after operator GO on target DB.
...
- **Next gated slice (recommended):** `tenant-operational-organization-id-not-null-migrations-v1` (D4 — operator GO + explicit approval) or other P1/P2 per priority. **Not recommended next:** `people-management-ux-unification-instructor-route-split-v1`.
...
- **Next gated slice (recommended):** `tenant-operational-organization-id-not-null-migrations-v1` (D4) after operator GO. Route split **deferred (D4)**.
- **Deferred:** `tenant-operational-organization-id-backfill-apply-v1` — only when a future dry-run reports proposed rows; `tenant-operational-organization-id-not-null-migrations-v1` after operator GO + explicit D4 approval.
