...
| ----- | -------- | ------ |
| `gitlab-ci-database-isolation-v1` | **P0** operational safety | **ACTIVE** — GitLab `check` integration blocker after DEC-068. **IMPLEMENTED + VALIDATED IN-BRANCH**. Base/published `main` `14c3865372502f074941a1fc81a55b5ec7f1b589`. Remediation: `before_script` shell-export of loopback `DATABASE_URL` / `DIRECT_URL` before `pnpm install` / `pnpm check`. DEC-068 unchanged (no CI remote-DB escape). Zero DB / zero migration. GitLab hosted pipeline **PENDING** external proof. Do not claim pipeline green from local evidence. Solution #4 `migration-deploy-target-safety-gate-v1` remains published at `ec39e05d2ffda40f439f3bdadc7889853e5ca9ef` and must not be modified; its merge remains blocked until this slice is GitLab-green and on `main`. |
...
- **Active implementation slice:** `gitlab-ci-database-isolation-v1` — **IMPLEMENTED + VALIDATED IN-BRANCH**; **NOT YET INTEGRATED ON MAIN**. Base / published `main` `14c3865372502f074941a1fc81a55b5ec7f1b589`. GitLab hosted pipeline **PENDING** external proof. DEC-068 unchanged. Solution #4 remains published at `ec39e05d2ffda40f439f3bdadc7889853e5ca9ef`; merge blocked until this CI slice is GitLab-green and on `main`.
