...
- **Done (v1 slice):** `instructor-hard-delete-zero-deps-v1` — Zero-deps hard delete: `DELETE /api/admin/instructors/[id]`; policy/service; Profiles Delete allowed/blocked UI; legacy `DELETE /api/users/delete` guard for INSTRUCTOR. Policy: [instructor-delete-policy.md](./instructor-delete-policy.md). Deactivate + app accounts demote still deferred. Validated via `pnpm check`.
- **People polish (next / deferred):** `instructor-deactivate-v1`, `people-management-instructor-app-access-lifecycle-policy-v1`, `people-management-app-accounts-demote-v1` (after instructor deactivate; hard delete + legacy guard done), `people-management-student-email-change-policy-v1` (canonical Change email flow), `people-management-app-account-unlink-policy-v1`, `people-management-onboarding-unlinked-invitations-v1`, `people-management-instructor-profile-invite-badge-v1`, `instructor-records-list-api-v1`, `admin-users-page-tenant-scope-v1`, `instructor-records-export-v1`, `instructor-records-import-dry-run-v1`, `cleanup-test-people-dry-run-v1` — see parent note `people-management-records-polish-v1` (analysis only). **Absorbed/done:** `people-management-record-status-badges-v1`, `instructor-people-search-v1`, `people-management-app-access-lifecycle-policy-v1` (analysis → remove + reactivate implemented), `instructor-hard-delete-zero-deps-v1`.
