...
- **Done (B1 slice):** `supabase-rls-class-b-hardening-v1b-nextauth-v1` — migration `20260610150000_supabase_rls_class_b_hardening_v1b_nextauth`: RLS + REVOKE on `accounts`, `sessions`, `verification_tokens`, `users` only; no `CREATE POLICY`, no `FORCE ROW LEVEL SECURITY`, no Prisma schema/runtime changes. Operator `migrate deploy` human-controlled. Validated via `pnpm check`.
- **Next gated code slice (recommended):** `supabase-rls-class-b-hardening-v1b-tenant-business-revoke-v1` (D4 — B2: 12 tenant/platform tables). **Prerequisite:** B1 deployed and smoke-passed on target env. **Not recommended next:** monolithic v1b (19 tables) or `people-management-ux-unification-instructor-route-split-v1`.
