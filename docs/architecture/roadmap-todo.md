...
| ----- | -------- | ------ |
| Solution #9 / `license-key-security-hardening-v1` / `LICENSING-002` | **P1 commercial security** | **ACTIVE** — finding `LICENSING-002` **OPEN**. Architecture **PENDING INTAKE / ARCHITECT REVIEW**. Implementation **NOT AUTHORIZED**. Schema/migration **UNDETERMINED**. No remote DB / Production mutation authorized. Remediation objective: cryptographic key generation + atomic single-use consumption + concurrency-safe idempotent entitlement effects. Ordered after #9 (not activated): `LICENSING-001` → `AUTH-RATE-001` → `TEST-GATE-001`. |
...
| #8 | `jwt-session-revocation-v1` / `AUTH-SESSION-001` | **CLOSED AND CLEANED UP**. Finding `AUTH-SESSION-001` **CLOSED**. Architecture `USER_SESSION_VERSION_EPOCH`. Framework authority NextAuth **4.24.15**. Implementation anchor `5af60d259d9f394590f926ceaaacf6f454aac282`. Closure anchor `dadecd8c3103fbae53a36ae22f012545515e4ab0`. Migration `20260903120000_add_user_auth_session_version` **APPLIED** on Production by human (DEC-069). Main technical pipeline `2819681959` / IID **389** SUCCESS. Closure pipeline `2819992756` / IID **390** SUCCESS. Production read-only smoke **PASS**. Same migrate deploy also applied historical `20260714160000_platform_commercial_catalog_schema_foundation_v1` (governance deviation; technical risk LOW; no commercial seed). Queued follow-ups (inactive): `migration-deploy-pending-set-enumeration-v1`; `production-smoke-hosted-timeout-hardening-v1`. Source branch **ABSENT** local + remote |
| #9 | `license-key-security-hardening-v1` / `LICENSING-002` | **ACTIVE**. Finding `LICENSING-002` **OPEN**. Priority **P1**. Architecture **PENDING INTAKE / ARCHITECT REVIEW**. Implementation **NOT AUTHORIZED**. Schema/migration **UNDETERMINED**. Remote DB / Production **NOT AUTHORIZED**. Accepted defects: weak key generation; pre-transaction `isUsed` check; no atomic conditional consume; concurrent duplicate entitlement risk. Remediation objective: `CRYPTOGRAPHIC_KEY_GENERATION` + `ATOMIC_SINGLE_USE_CONSUMPTION` + `CONCURRENCY_SAFE_IDEMPOTENT_ENTITLEMENT_EFFECTS` |

DAT_4.6 conversation/context remains healthy. Solution #8 is **CLOSED AND CLEANED UP**. Solution #9 / `LICENSING-002` is the **only** active canonical engineering Solution. No handoff to DAT_4.7 is currently required. Implementation of Solution #9 is **NOT AUTHORIZED** until architect architecture review and explicit GO. Ordered after #9 (not activated): `LICENSING-001` → `AUTH-RATE-001` → `TEST-GATE-001`.
...
## P1 / Engineering excellence — closure gate and post-audit execution
...
- **Status:** **CLOSED AND CLEANED UP**. Finding `AUTH-SESSION-001` **CLOSED**.
- **Implementation anchor:** `5af60d259d9f394590f926ceaaacf6f454aac282`.
- **Closure anchor:** `dadecd8c3103fbae53a36ae22f012545515e4ab0`.
...
- **Validation:** hosted source pipeline `2816551646` / IID **388** SUCCESS; main technical pipeline `2819681959` / IID **389** SUCCESS; closure pipeline `2819992756` / IID **390** SUCCESS; Production read-only smoke **PASS** (fixture 1/0/0 in 42.0s; API exit 0; readonly UI 4/0/0; mutation smoke NOT RUN).
- **Source branch:** **ABSENT** local + remote.
...
- **Solution:** #9 — **ACTIVE**.
- **Finding status:** **OPEN** (do not claim remediated).
- **Architecture:** **PENDING INTAKE / ARCHITECT REVIEW** (not accepted).
- **Implementation:** **NOT AUTHORIZED**.
- **Schema/migration:** **UNDETERMINED**.
- **Remote DB / Production:** **NOT AUTHORIZED**.
- Accepted finding statement: weak/non-cryptographic key generation; pre-transaction single-use check; no atomic conditional consume; concurrent activation can attempt duplicate entitlement effects.
- Canonical remediation objective: cryptographically secure license-key generation; atomic single-use consumption; concurrency-safe / idempotent entitlement effects; add concurrent activation tests.
- `LicenseKey.key` `@unique` alone does **not** close `LICENSING-002`.
...
- **Classification:** **QUEUED ENGINEERING PROCESS HARDENING — NON-BLOCKING**
- **Problem observed (Solution #5):** the pre-commit hook ran lint-staged/formatter after the independent EQR snapshot had already been produced, causing formatter-only changes to EQR-reviewed source and requiring a second integrity and exact-commit validation cycle.
- **Current Solution #5 result:** the hook delta was independently reviewed as formatting-only, and the exact committed source was subsequently fully revalidated (`9e59712…` / tree `607d720…`).
- **Future excellence rule:** for future DAT Solutions, run the repository's canonical formatter / lint-staged-equivalent normalization **before** the final EQR snapshot/hash anchor.
...
| `AUTH-SESSION-001` | `jwt-session-revocation-v1` (Solution #8 — **CLOSED AND CLEANED UP**; finding **CLOSED**; implementation `5af60d259d9f394590f926ceaaacf6f454aac282`; closure anchor `dadecd8c3103fbae53a36ae22f012545515e4ab0`; main pipeline `2819681959` / IID **389** SUCCESS; closure pipeline `2819992756` / IID **390** SUCCESS; Production read-only smoke PASS; source **ABSENT** local + remote) |
| `AUTH-LINK-001` | `auth-email-link-trusted-origin-v1` |
| `PLATFORM-ATOM-001` | `platform-onboarding-transaction-boundary-v1` |
| `LICENSING-002` | `license-key-security-hardening-v1` (Solution #9 — **ACTIVE**; finding **OPEN**; architecture pending; implementation **NOT AUTHORIZED**) |
...
| `TEST-HYGIENE-001` | `e2e-suite-contract-repair-v1` (**CLOSED** — Solution #6 **CLOSED ON PUBLISHED MAIN**; implementation `c9bd18e…`; hosted main pipeline `2812557887` / IID **382** SUCCESS) |
...
| `CONFIG-ENV-001` | `local-development-database-isolation-v1` (**CLOSED ON PUBLISHED MAIN** at `14c3865372502f074941a1fc81a55b5ec7f1b589`, DEC-068; implementation anchor `a717534ed351440fdfbf6800b218d56d6eb85282`; accepted tree `5b9aa29649bdf929bf7df5f9f641eb950ce16275`; continuity anchor `810cc9446c5f89805e282672bc03f743f8480d75`. Historical in-branch: IMPLEMENTED + VALIDATED; SOURCE BRANCH PUBLISHED; NOT YET INTEGRATED ON MAIN) |

| `UI-DATA-001` | `dashboard-statistics-contract-alignment-v1` |

| `DB-MIGRATION-001` | `migration-deploy-target-safety-gate-v1` (**CLOSED ON PUBLISHED MAIN**; DEC-069; terminal source `88eb0ab3637bf0b18f23271cb88976bc3e1f1fae`; terminal hosted pipeline **#2786482679** / IID **374** / job **#16075003942** SUCCESS; local `main` integration `8653c56824319017ba5ec3967706e5e254d280a7`; tree `36a36f25d81d94792ab1e70d4d26689c1c545a7e`; published-main validation anchor `4c74d6a714e68492dbcf535ca0527f01131eba66`; hosted main pipeline **#2789017209** / IID **375** / job **#16092802312** SUCCESS; local validation 211/211 files, 1805/1805 tests PASS; pre-publication remote-main baseline `14b07dfc2ece5100dfee9b5f1c56f9ee0ab09ebe`; architect EQR B1 **CLOSED**; architect v2 EQR **PASS**; implementation `10093da1068f6d08caf0bb5f83c6810429dd61d7`; recovery `47a101b261a2fab144d73469e699389fc94ceb1c`; post-implementation continuity/source-publication anchor `6d178bc3479668448809ee861c54fa3a0074cb46`. Historical (superseded): current-main reconciliation merge `590e0aed27d30bc331b54a5af8b18c230215ca6f`; hosted-validated source checkpoint `ef705010e46e3eb8c3c1232a9b9f09c3103d7f74`; earlier live wording **LOCAL MAIN INTEGRATION CHECKPOINT / PUBLICATION + HOSTED VALIDATION REQUIRED BEFORE CLOSURE**. No migration executed. Source-branch cleanup is separate human-gated housekeeping.) |
...
- Historical Wave A3 note: browser/edge cookie evidence alone did **not** close `AUTH-SESSION-001`. **Current truth:** Solution #8 / `jwt-session-revocation-v1` **CLOSED AND CLEANED UP** (`AUTH-SESSION-001` **CLOSED**; implementation `5af60d259d9f394590f926ceaaacf6f454aac282`; closure anchor `dadecd8c3103fbae53a36ae22f012545515e4ab0`; source **ABSENT** local + remote).
...
<!-- post-audit-execution-priority-queue-v1 -->
## Post-audit execution priority queue

This queue is an execution overlay on the authoritative **Master remediation
ledger — 51 findings**. It does not replace or duplicate finding ownership.
...
2. `e2e-suite-contract-repair-v1` — **CLOSED ON PUBLISHED MAIN** — implementation `c9bd18e…`; hosted main pipeline `2812557887` / IID **382** **SUCCESS**; `TEST-HYGIENE-001` **CLOSED**

These are confidence multipliers for subsequent security, billing, integrity and
migration work.

### P1-B — Core security and integrity
...
- `jwt-session-revocation-v1` — **CLOSED AND CLEANED UP** (Solution #8 / `AUTH-SESSION-001` **CLOSED**; implementation `5af60d259d9f394590f926ceaaacf6f454aac282`; closure anchor `dadecd8c3103fbae53a36ae22f012545515e4ab0`; source **ABSENT** local + remote)
- `license-key-security-hardening-v1` — **ACTIVE** (Solution #9 / `LICENSING-002` **OPEN**; architecture pending; implementation **NOT AUTHORIZED**)
- `provider-owned-entitlement-mutation-boundary-v1`
- `nextauth-credentials-rate-limit-alignment-v1`
- `auth-email-link-trusted-origin-v1`
- `privileged-password-policy-alignment-v1`
...
### P1-C — Client-visible correctness and operational value

Prioritize independently safe slices that produce visible customer value:

- `people-instructor-invite-accept-list-refresh-v1`
- `lesson-edit-contract-alignment-v1`
- `dashboard-statistics-contract-alignment-v1`
- `student-profile-atomic-update-v1`
- `instructor-profile-atomic-update-v1`
- `generic-user-update-contract-narrowing-v1`

### P1 parallel architecture lane

`platform-separation-architecture-plan-v1` starts after formal audit closure and
may proceed in parallel with independent P0 containment / proof work.

Urgent P0 containment is not blocked by the architecture plan.

Deeper structural refactors that could affect Platform/DAT boundaries should
wait for this architecture plan.

### P2 — Engineering-quality consolidation

After higher-risk and customer-visible P1 work is sufficiently controlled:

- API/UI orchestration and duplication consolidation;
- large-component decomposition;
- accessibility remediation and regression sweep;
- notification-stack consolidation;
- toolchain/dependency responsibility pruning;
- legacy in-memory rate-limit dormant-contract retirement;
- remaining code-hygiene and test-gate work.

### P2/P3 — Data modernization and staged retirement

Legacy-model/schema retirement remains staged and safety-gated.

Do not perform schema/data retirement until the relevant local-development
isolation, migration-authority, migration-review, rollback and human approval
contracts are satisfied.

### Execution governance

- one smallest-safe behavioural scope per working branch;
- evidence and prerequisites before mutation;
- `pnpm -C driving_school_platform/nextjs_space check` remains the canonical
  local validation command;
- proportional Engineering Quality Review for every implementation slice;
- no push, merge, remote branch deletion, database write, hosted mutation,
  production mutation or behavioural scope expansion without the required
...
- **Conversation-recovery note (2026-08-20, historical):** part of the external ChatGPT conversation became unavailable after Solution #3 work. Live Git evidence is authoritative. That recovery reconciled canonical docs to published Git state. No product behavior was changed by the recovery itself.
- Solution #3 / `local-development-database-isolation-v1` (source `CONFIG-ENV-001`): **CLOSED ON PUBLISHED MAIN** at `14c3865372502f074941a1fc81a55b5ec7f1b589` (DEC-068; historical Solution #3 merge, not current live `main`). Implementation anchor `a717534ed351440fdfbf6800b218d56d6eb85282`; accepted tree `5b9aa29649bdf929bf7df5f9f641eb950ce16275`; continuity anchor `810cc9446c5f89805e282672bc03f743f8480d75`. Source branch cleanup **COMPLETE**. Historical in-branch wording “ACTIVE / NOT YET INTEGRATED ON MAIN” is superseded.
- Current published `main`: resolve `origin/main` through live Git. Solution #4 published-main validation anchor: `4c74d6a714e68492dbcf535ca0527f01131eba66` — not an eternal current HEAD. Pre-publication remote-main baseline: `14b07dfc2ece5100dfee9b5f1c56f9ee0ab09ebe`. CI-isolation published integration anchor: `594484998da5893b349bea4d2de1ac7447d37de6`. CI-isolation closure-doc publication anchor: `14b07dfc2ece5100dfee9b5f1c56f9ee0ab09ebe`.
- **Closed CI isolation slice:** `gitlab-ci-database-isolation-v1` — **CLOSED ON PUBLISHED MAIN** at `594484998da5893b349bea4d2de1ac7447d37de6`. Implementation anchor `9c88e3942876d65ae1290819151022cf600aefa5`. Final source continuity anchor `ac52061a1b1e972775ecc3212e43d8f0f0688fc9`. Final closure hosted main pipeline **#2780314232** SUCCESS; canonical job `check` **#16037631071** SUCCESS; `allow_failure: false`. Hosted CI isolation criterion **SATISFIED**. DEC-068 unchanged (no `CI` / `GITLAB_CI` remote-DB escape). No DB or migration was performed as part of CI isolation. Source branch cleanup **COMPLETE**, local and remote.
- **Closed operational slice:** Solution #4 / `migration-deploy-target-safety-gate-v1` (source `DB-MIGRATION-001`) — **CLOSED ON PUBLISHED MAIN** (DEC-069). Terminal source `88eb0ab3637bf0b18f23271cb88976bc3e1f1fae`. Terminal hosted pipeline **#2786482679** (IID **374**) **SUCCESS**; job `check` **#16075003942** **SUCCESS**. Local `main` integration `8653c56824319017ba5ec3967706e5e254d280a7` (tree `36a36f25d81d94792ab1e70d4d26689c1c545a7e`; parents `14b07dfc2ece5100dfee9b5f1c56f9ee0ab09ebe` + `88eb0ab3637bf0b18f23271cb88976bc3e1f1fae`). Published-main validation anchor `4c74d6a714e68492dbcf535ca0527f01131eba66`. Hosted main pipeline **#2789017209** (IID **375**) **SUCCESS**; job `check` **#16092802312** **SUCCESS**. Local integration validation PASS: 211/211 files, 1805/1805 tests. Pre-publication remote-main baseline `14b07dfc2ece5100dfee9b5f1c56f9ee0ab09ebe`. Historical (superseded): `ef705010e46e3eb8c3c1232a9b9f09c3103d7f74`; `590e0aed27d30bc331b54a5af8b18c230215ca6f`; earlier wording **SOURCE CHECKPOINT PUBLISHED + HOSTED-VALIDATED / TERMINAL SOURCE-CONTINUITY CHECKPOINT / MAIN INTEGRATION PENDING** and **LOCAL MAIN INTEGRATION CHECKPOINT / PUBLICATION + HOSTED VALIDATION REQUIRED BEFORE CLOSURE**. Durable anchors: recovery `47a101b261a2fab144d73469e699389fc94ceb1c`; implementation `10093da1068f6d08caf0bb5f83c6810429dd61d7`; post-implementation continuity `6d178bc3479668448809ee861c54fa3a0074cb46`. Architect EQR B1 **CLOSED**; architect v2 EQR **PASS** (historical in-branch). No migration has been run. Source-branch cleanup is separate human-gated housekeeping; at this checkpoint the source branch remained present at `88eb0ab3637bf0b18f23271cb88976bc3e1f1fae`. Do not start Solution #5.
