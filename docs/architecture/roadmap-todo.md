...
| `dat-production-smoke-canonical-fixtures-v1` | P0 / ops **in progress (repo)** | DEC-064 corrected: invite fixtures require real ACCEPTED + operator-only emails (blocker if missing); Sarah/Bob preserved additional; manual+negative rename targets; **`--apply` blocked until invites accepted** |
