...
| `dat-production-smoke-reconcile-v1` | P0 / ops **done (inspect tooling)** | DEC-063: smoke-only reconcile; **no** embedded Platform-admin restore; inspect tooling in repo (`pnpm ops:inspect-production-smoke-reconciliation`) |
| `dat-production-smoke-canonical-fixtures-v1` | P0 / ops **in progress** | DEC-064: real invites accepted (2026-07-24, human); Instructor 2 invite-resolvable + category B present; Student 2 category **B** (initial diagnosis C+E → operator changed to B via app); **sole remaining invite blocker** `UserInvitation.studentId=null` → `canonical_invited_student_missing`; features/plates legacy until `--apply`; Sarah/Bob preserved; **`--apply` not authorized**; see P0 repair slice below |
| `student-invite-accept-student-link-repair-v1` | **P0** | **Blocks closing** `dat-production-smoke-canonical-fixtures-v1`. Scope: (1) fix future unlinked STUDENT accept so `UserInvitation.studentId` is set in the same transaction as `acceptedUserId`/status; (2) operator-safe target-guarded idempotent repair for the single already-accepted technical invite; (3) tests for linked + unlinked + verification. Unlinked accept creates `Student` but leaves `studentId` null (`invitation-accept-service.ts`); reconciler correctly fail-closed (do **not** relax). Coherence of email/role/org; full rollback; audit log when applicable. **Not in scope:** category change (Student 2 already **B**). No direct Supabase repair authorized; **do not execute repair in docs batches**. |
...
| `platform-separation-architecture-plan-v1` | P1 backlog | **Recommended next after smoke fixtures close** — autonomous MeEngine Platform vs DAT boundary plan; embedded `/platform` transitional |
...
**Product direction (backlog — commercial path):** **Mobile/tablet** — **done**. **Competitive discovery** — **done**. **Commercial cutline planning** — **done** (`dat-v1-commercial-platform-cutline-plan-v1`, DEC-046–057). **Plan display names + memory continuity** — **done** (`dat-plan-naming-and-doc-hygiene-v1`, DEC-058/059). **Catalogue schema plan** — **done** (`platform-commercial-catalog-schema-plan-v1`, DEC-060). **Catalogue schema foundation** — **done (repo)** (`platform-commercial-catalog-schema-foundation-v1`, DEC-061). **Catalogue seed code** — **done (repo)** (`platform-commercial-catalog-seed-v1`; not executed; DEC-062 local-only legacy seed). **Smoke identity** — **done** (DEC-045) then **incident reset 2026-07-17** (test-only; IDs stale). **DEC-063:** do not recreate embedded Platform Admin; smoke-only inspect via `dat-production-smoke-reconcile-v1`. **DEC-064 fixtures in progress:** real invites accepted 2026-07-24; Instructor 2 ready for re-inspect; Student 2 blocked by null invitation `studentId` — **P0 next** `student-invite-accept-student-link-repair-v1`; P1 People list bug `people-instructor-invite-accept-list-refresh-v1`; `--apply` still unauthorized. **DEC-065 / identifiers:** `school-person-identifiers-settings-product-plan-v1` (plan Settings identifiers product; not implement numbering system yet). **Recommended next after smoke fixtures close:** `platform-separation-architecture-plan-v1` (not blind catalogue read-services inside embedded Platform).
