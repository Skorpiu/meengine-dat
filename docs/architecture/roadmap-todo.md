...
| `platform-admin-access-and-smoke-reconcile-v1` | P0 / ops | **Recommended next** — read-only remote validation; Platform admin reconcile; rotate seeded credentials; capture new `DAT_SMOKE_*` IDs; no catalogue migrate/apply |
| `platform-commercial-catalog-read-services-v1` | P1 backlog | Read-only catalogue repositories; no writes/publication/checkout/provider — resume after smoke reconcile |
