...
- **Done (B3 slice):** `supabase-rls-class-b-hardening-v1b-global-reference-v1` — migration `20260610170000_supabase_rls_class_b_hardening_v1b_global_reference`: RLS + REVOKE on `categories`, `transmission_types`, `user_preferences` only; no `CREATE POLICY`, no `FORCE ROW LEVEL SECURITY`, no Prisma schema/runtime changes. Baseline main `018812a`. Operator `migrate deploy` human-controlled. Validated via `pnpm check`. **v1b revoke-only complete (31/31).**
- **Next (docs-only, recommended):** `supabase-rls-class-b-hardening-v1b-global-reference-deploy-record-v1` — deploy + smoke after B3 operator deploy. **Not recommended next:** monolithic v1b or `people-management-ux-unification-instructor-route-split-v1`.
