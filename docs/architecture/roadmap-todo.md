...
- **Done (v1 slice):** `people-management-app-accounts-demote-v1` — App Accounts removed from People L1 tabs; **Advanced accounts** read-only diagnostics section (collapsed by default); removed New/Edit/Delete and stats cards; APIs unchanged. Constants: `lib/people/people-management-ui.ts`. Validated via `pnpm check`.
