...
| `dat-production-smoke-canonical-fixtures-v1` | P0 / ops **in progress** | DEC-064: real invites accepted (2026-07-24, human); Instructor 2 invite-resolvable + category B present; Student 2 category **B**; **sole remaining invite blocker** was `UserInvitation.studentId=null` — accept-path + repair CLI shipped in `student-invite-accept-student-link-repair-v1` (**remote repair apply pending human**); features/plates legacy until fixture `--apply`; Sarah/Bob preserved; fixture **`--apply` not authorized** until repair verified via dry-run |
| `student-invite-accept-student-link-repair-v1` | **Done (repo)** — remote repair apply **pending human** | Accept path persists `UserInvitation.studentId` in the same transaction (`invitation-accept-service.ts`). Operator CLI `pnpm ops:repair-accepted-student-invitation-link` (dry-run default; `-- --apply` writes **only** `studentId`; target guard; smoke org; email from `DAT_SMOKE_INVITED_STUDENT_EMAIL` only). Reconciler stays fail-closed. **Agent did not run remote dry-run/apply.** Category already B (not in scope). |
