...
- **Done (v1 slice):** `people-management-app-access-remove-v1` — **Remove app access** for APP_USER students: `POST /api/admin/students/[id]/app-access/remove`; transactional service; Edit Student → App access UI + confirmation; preserves profile/history; disables login (`isApproved=false`, session invalidation); Student → `MANUAL_ONLY` + `userId=null`. Policy: [student-app-access-lifecycle-policy.md](./student-app-access-lifecycle-policy.md). Reactivate/Change email/App accounts demote still deferred. Validated via `pnpm check`.
- **People polish (split / deferred):** `people-management-edit-instructor-unified-v1`, `people-management-app-access-reactivate-v1`, `people-management-app-accounts-demote-v1` (after remove/reactivate parity), `people-management-student-email-change-policy-v1` (canonical Change email flow), `people-management-app-account-unlink-policy-v1`, `people-management-onboarding-unlinked-invitations-v1`, `people-management-instructor-profile-invite-badge-v1`, `instructor-records-list-api-v1`, `admin-users-page-tenant-scope-v1`, `instructor-records-export-v1`, `instructor-records-import-dry-run-v1`, `instructor-delete-policy-v1`, `cleanup-test-people-dry-run-v1` — see parent note `people-management-records-polish-v1` (analysis only). **Absorbed/done:** `people-management-record-status-badges-v1`, `instructor-people-search-v1`, `people-management-app-access-lifecycle-policy-v1` (analysis → remove v1 implemented).
...
- **Next gated slice (recommended):** `admin-settings-client-visibility-review-v1` (product P1) or `supabase-rls-class-b-hardening-v1b` (D4 — remaining internal tables — separate approval). **Not recommended next:** `people-management-ux-unification-instructor-route-split-v1`.
...
- **Next gated slice (recommended):** `admin-settings-client-visibility-review-v1` or other P1 per priority. Route split **deferred (D4)**.
