...
| `dat-production-smoke-canonical-fixtures-v1` | P0 / ops **in progress (repo)** | DEC-064: canonical names + smoke feature overrides + `pnpm ops:reconcile-production-smoke-fixtures` (dry-run/`--apply`); seed + inspector updated; **remote `--apply` not executed by agent** |
