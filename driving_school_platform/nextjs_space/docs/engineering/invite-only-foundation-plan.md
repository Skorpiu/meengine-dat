...
### Batch 4: `invitation-accept-flow`

|                         |                                                                                       |
| ----------------------- | ------------------------------------------------------------------------------------- |
| **Objective**           | Public GET/POST accept; user + profile transaction; mark `ACCEPTED`.                  |
| **Risks**               | Race on double accept; enumeration via email errors; host/org mismatch.               |
| **Tests**               | Accept happy path; expired/revoked/invalid; wrong host; instructor required fields.   |
| **Acceptance criteria** | Invitee can complete account without admin password handoff; one-time token enforced. |
