...
| **Invite-only foundation**            | **Partial**     | Schema (batch 1) + token service [`lib/invitations/invitation-token-service.ts`](../../lib/invitations/invitation-token-service.ts) (batch 2). **No APIs/UI yet.** Today: School Admin `/api/users/create`; no invite flow. |
