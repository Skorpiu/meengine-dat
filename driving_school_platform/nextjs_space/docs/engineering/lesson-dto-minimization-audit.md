...
| **Query**  | `view` maps to `lessonType` (`DRIVING` → `DRIVING`, `CODE` → `THEORY`, `EXAMS` → `EXAM`) |
...
| **student.user**    | Full `User` row via `user: true`                                                                                                                          | Includes **`passwordHash`**, tokens, `email`, address — **security risk** on list/calendar/detail GETs           |
| **instructor**      | Full `Instructor` row                                                                                                                                     | UI uses nested `user` only; license numbers, rates, JSON working hours unused on calendar                        |
| **instructor.user** | Full `User` row                                                                                                                                           | Same **`passwordHash`** exposure as student                                                                      |
...
| `components/admin/lessons-management-client.tsx`                  | `GET /api/admin/lessons?view=`                           | Intended: `recent`, `current`, `upcoming`                  | **Lesson:** `id`, `lessonDate`, `startTime`, `status`. **student.user:** `firstName`, `lastName`. **instructor.user:** `firstName`, `lastName`. **vehicle:** `registrationNumber`, `make`, `model`. **category:** `name`. **EXAMS UI (typed):** `examDate`, `examType`, `examLocation`, `examiner.user` | **Bug:** client reads `recent`/`current`/`upcoming` at **root**; API returns them under `data` inside `successResponse` — lists likely empty in production (see LD-002). EXAMS fields are **not** on `Lesson` model (see LD-003). |
...
| **Dashboard envelope mismatch**        | **High (functional)** | `lessons-management-client` does not read `data.recent` / `data.current` / `data.upcoming`. Fix before or as part of DTO work.                                                       |
| **EXAMS view field mismatch**          | **Medium**            | UI references `examDate`, `examType`, `examLocation`, `examiner` but dashboard query returns `Lesson` rows with `lessonType: "EXAM"` only — separate `Exam` model exists in schema.  |
...
| 5    | **dashboard-dto**        | Fix `lessons-management-client` envelope parsing; apply `DashboardLessonDto`; resolve EXAMS data source.         |
...
| ------ | -------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| LD-001 | **P1**   | Lesson list/calendar/detail responses can include **`passwordHash`** via `include: { user: true }`.                                          | `LESSON_LIST_INCLUDE`, `[id]/route.ts` GET `include`, Prisma `User` model                                     | Step 3: `select` only safe user fields; contract test forbids `passwordHash`.                                          |
| LD-002 | **P1**   | Admin lesson management UI likely **never populates lists** — reads `recent` at JSON root, API returns `{ success, data: { recent, ... } }`. | `lessons-management-client.tsx` L100–106; `route.integration.unit.test.ts` L136–142                           | Fix client unwrap in dashboard-dto batch (no API change required) or document if intentional.                          |
| LD-003 | **P2**   | **EXAMS** management view expects **Exam-shaped** fields; API returns **Lesson** rows for `lessonType: "EXAM"`.                              | `lessons-management-client.tsx` EXAMS branch; `getAdminDashboardLessons` EXAMS branch; `Exam` model in schema | Product decision: map Lesson → display DTO or fetch `Exam` registrations; do not strip fields until source is correct. |
