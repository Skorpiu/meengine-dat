...
| **Public signup**      | **`POST /api/signup`** blocks **`Organization.isDemo`** with **403** / `demo_signup_disabled`. **Public signup disabled by default:** **`PUBLIC_SIGNUP_ENABLED=true`** required for non-demo orgs else **403** / `public_signup_disabled` ([signup-hardening-plan.md](./signup-hardening-plan.md)). **Still open:** distributed rate limiting (not in-memory), captcha, real email verification. `isEmailVerified` remains a placeholder. |
...
| EEA-007 | P2       | Security / abuse | **Partially addressed:** demo block + **`PUBLIC_SIGNUP_ENABLED`** gate (default off). **Pending:** distributed rate limit (not in-memory), invite-only foundation, **real email verification**, optional captcha; `isEmailVerified` remains a placeholder.                                                                                                                              | `app/api/signup/route.ts`, `lib/signup/signup-policy.ts`, [signup-hardening-plan.md](./signup-hardening-plan.md)                         | Remaining signup-hardening batches when self-serve is in scope.                                                     |
...
- **Signup / public forms:** demo block + **`PUBLIC_SIGNUP_ENABLED`** (default off). **Still open:** invite-only, distributed **rate limits**, **email verification**, optional **captcha** — [signup-hardening-plan.md](./signup-hardening-plan.md), [dat-production-readiness-gaps.md](../ops/dat-production-readiness-gaps.md).
