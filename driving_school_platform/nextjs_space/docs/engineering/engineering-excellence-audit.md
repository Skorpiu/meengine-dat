...
| **Playwright**                   | `@playwright/test` present with `playwright.config.ts`, specs under `tests/` (e.g. vehicles gating, theory exam), and **demo smoke foundation** at `e2e/demo-smoke.spec.ts` ([e2e-smoke.md](../ops/e2e-smoke.md)). **Not** part of `pnpm check` or GitLab CI—E2E is **optional / manual** unless CI is extended. |
...
| EEA-008 | P3       | Testing          | **Foundation started:** optional demo smoke (`e2e/demo-smoke.spec.ts`, `pnpm test:e2e:demo`, [e2e-smoke.md](../ops/e2e-smoke.md)); still outside `pnpm check` and GitLab CI. Legacy specs under `tests/` unchanged.                                                                                                                                                                     | `playwright.config.ts`, `e2e/demo-smoke.spec.ts`, `tests/*.spec.ts`, `.gitlab-ci.yml`                                                    | Add optional CI job when operators want automated post-deploy smoke on staging.                                     |
...
- Optional **E2E in CI** for smoke-critical flows—**demo read-only smoke foundation** exists ([e2e-smoke.md](../ops/e2e-smoke.md)); wire an optional job when staging credentials are available.
