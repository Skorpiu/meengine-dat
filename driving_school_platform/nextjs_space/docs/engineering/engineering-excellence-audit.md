...
| **Demo UX polish**                                | Schedule Map **day-view refetch** fixed (`validateLessonCalendarRange` accepts single-day `from`/`to`); booking refreshes calendar in place with compact chips; lesson-type colors: theory green, driving blue, `THEORY_EXAM` yellow, `EXAM` orange (`schedule-map-refresh-and-layout-fix`).         |
...
| EEA-002 | P2       | Data / API       | **Partially addressed:** calendar range guardrails + lesson nested user select (LD-001). **Still open:** broader DTO field trim; row `take`/cursor if payloads grow.                                                                                                                                                     | `lib/lessons/calendar-range.ts`, `lib/users/user-public-select.ts`            | Batch 6 per [lesson-dto-minimization-audit.md](./lesson-dto-minimization-audit.md); pagination if load tests warrant. |
...
- Execute [lessons-route-refactor-plan.md](./lessons-route-refactor-plan.md) in small PRs (queries → mappers → create → update/delete → shared calendar reads); keep **GET** strictly read-only.
