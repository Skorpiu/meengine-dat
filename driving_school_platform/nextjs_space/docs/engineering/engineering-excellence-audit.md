...
| EEA-002 | P2       | Data / API       | **Partially addressed:** admin/instructor/student calendar `from`/`to` reject invalid ranges and spans **> 90 days** (`validateLessonCalendarRange`). **Still open:** row `take`/cursor inside the 90-day window if payloads grow.                                                                                       | `lib/lessons/calendar-range.ts`, `app/api/*/lessons/route.ts`                 | Optional batch 6 DTO minimization; pagination if load tests warrant.                                                |
