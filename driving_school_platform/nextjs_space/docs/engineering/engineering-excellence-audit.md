...
| **Public signup**      | **`POST /api/signup`** blocks **`Organization.isDemo`** tenants with **403** / `demo_signup_disabled`. **Not** wired to in-memory `RATE_LIMITS`—**edge/distributed rate limiting**, **captcha**, and **real email verification** remain **P1** before broad public self-serve signup on serverless. **Signup hardening plan created:** [signup-hardening-plan.md](./signup-hardening-plan.md) (phased invite-only → verification → distributed limits → Turnstile; implementation pending). `isEmailVerified` is still a placeholder until verification ships. |
...
| EEA-007 | P2       | Security / abuse | **Partially addressed:** demo orgs cannot use public signup (**403** / `demo_signup_disabled`). **Plan created:** [signup-hardening-plan.md](./signup-hardening-plan.md). **Implementation pending:** distributed rate limit (not in-memory on serverless), invite-only / public-disable config, **real email verification**, optional captcha; `isEmailVerified` remains a placeholder. | `app/api/signup/route.ts`, [signup-hardening-plan.md](./signup-hardening-plan.md)                                                        | Execute phased batches in plan when self-serve signup is in scope.                                                  |
...
- **Signup / public forms:** demo-tenant **public signup is blocked** in API (`demo_signup_disabled`). **Plan:** [signup-hardening-plan.md](./signup-hardening-plan.md). **Implementation pending:** env/config public-disable or invite-only, distributed **rate limits**, **email verification**, optional **captcha** before marketing broad self-serve registration (see [dat-production-readiness-gaps.md](../ops/dat-production-readiness-gaps.md)).
...
- [signup-hardening-plan.md](./signup-hardening-plan.md) — phased signup abuse controls (plan only; implementation pending)
