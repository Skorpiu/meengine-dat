...
| Area                                              | Observation                                                                                                                                                                                                                                                                                          |
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **CI and local gate**                             | Root GitLab pipeline runs `pnpm run test:run` and `pnpm run build` under `driving_school_platform/nextjs_space`. Developers align on **`pnpm check`** (`lint` + `typecheck` + `test:run` + `build`), which is stricter than the current CI script surface.                                           |
...
- **Domain modules:** Billing, licensing, platform onboarding/listing, demo policy, tenant resolution, and validation are **extracted**; lesson scheduling logic still mixes **orchestration** (multiple Prisma calls, cleanup side effect) inside `app/api/admin/lessons/route.ts`.
...
- Demo org flag + mutation guards align with ops runbooks. Automatic `cleanupOldLessons` invoked from admin lessons GET (non-demo orgs) couples **read path** to **destructive maintenance**—documented here under [Query and Big O review](#query-and-big-o-review) and [Findings](#findings).
...
| **Pagination**               | `admin/config-history` uses `take` / `skip` from query—**good**. `admin/lessons` dashboard slices use `take: 50` on time-window lists—**bounded**. **Calendar mode** (`from` + `to` query) uses `lesson.findMany` with date range and **no `take`**—large orgs/long windows could return **large payloads** (evidence: `app/api/admin/lessons/route.ts`). **Needs inspection** for max range / abuse. |
...
| **Automatic cleanup on GET** | `GET /api/admin/lessons` triggers `cleanupOldLessons` for non-demo orgs before reads—side effect on read path; idempotent but increases latency and write load under traffic.                                                                                                                                                                                                                         |
...
| **Public signup**      | `app/api/signup/route.ts` performs manual validation (no `withErrorHandling` / centralized rate limit in the opened portion)—**needs inspection** for abuse controls (compare to `RATE_LIMITS` usage elsewhere). |
...
| **Playwright**                   | `@playwright/test` present with `playwright.config.ts` and specs under `tests/` (e.g. vehicles gating, theory exam). **Not** part of `pnpm check` or the shown GitLab jobs—E2E is **optional / manual** unless CI is extended. |
...
| **CI vs local**     | `.gitlab-ci.yml` does **not** run `lint` or `typecheck` in the excerpted jobs—local `pnpm check` can catch issues CI misses until pipeline parity is improved. |
...
| ------- | -------- | ---------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| EEA-001 | P2       | CI               | Pipeline runs `test:run` + `build` but not the full `pnpm check` (lint + typecheck).                                                   | `.gitlab-ci.yml` `unit_tests` / `build` jobs                                   | Add `pnpm run lint` and `pnpm run typecheck` (or a single `pnpm run check`) to CI to match local gate, or document intentional split. |
| EEA-002 | P2       | Data / API       | Admin lessons **calendar** query returns all lessons in `[from, to)` with no `take` / pagination.                                      | `app/api/admin/lessons/route.ts` (`from`/`to` branch)                          | Cap rows, add pagination, or enforce max window size; load-test with large orgs.                                                      |
...
| EEA-004 | P3       | Architecture     | Read path (`GET /api/admin/lessons`) triggers `cleanupOldLessons` (writes) for non-demo orgs.                                          | `app/api/admin/lessons/route.ts`                                               | Move cleanup to cron/POST `/api/admin/cleanup` only, or feature-flag; measure latency impact.                                         |
...
| EEA-007 | P2       | Security / abuse | Public `POST /api/signup` uses manual parsing; rate limiting not verified in sampled code vs `withErrorHandling` pattern.              | `app/api/signup/route.ts`                                                      | Audit rate limits / captcha for public multi-tenant signup; align with `RATE_LIMITS` if product exposes this broadly.                 |
| EEA-008 | P3       | Testing          | Playwright specs exist but are outside `pnpm check` and shown CI stages.                                                               | `playwright.config.ts`, `tests/*.spec.ts`, `.gitlab-ci.yml`                    | Add optional CI job or document when operators run E2E (staging).                                                                     |
...
- Confirm **CI parity** with `pnpm check` or document why lint/typecheck are excluded.
- Review **billing webhook** exposure on Vercel (URL secrecy, no public staging without signatures).
- **Signup / public forms:** confirm rate limits and production `NODE_ENV` response shapes (aligns with [dat-production-readiness-gaps.md](../ops/dat-production-readiness-gaps.md) P1 themes).

### Product readiness

- Calendar and vehicle **list scalability** (caps, pagination, max date range).
...
- Extract **lesson query** orchestration from `admin/lessons` route into a service module; reduce read-path side effects.
