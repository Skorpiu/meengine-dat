...
| EEA-002 | P2       | Data / API       | **Partially addressed:** admin calendar `from`/`to` rejects invalid ranges and spans **> 90 days** (`invalid_calendar_range`, `calendar_range_too_large`). **Still open:** row `take`/cursor; student/instructor alignment — [lessons-route-refactor-plan.md](./lessons-route-refactor-plan.md).                         | `lib/lessons/calendar-range.ts`, `app/api/admin/lessons/route.ts`             | Execute plan batches 5–6 when load/product warrants.                                                                |
