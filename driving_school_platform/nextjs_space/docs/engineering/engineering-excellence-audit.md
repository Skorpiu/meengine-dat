...
| EEA-002 | P2       | Data / API       | **Partially addressed:** calendar range guardrails shipped. **Audit done:** [lesson-dto-minimization-audit.md](./lesson-dto-minimization-audit.md) (UI fields, `passwordHash` risk LD-001). **Still open:** contract-safe DTO implementation; row `take`/cursor if payloads grow.                                        | `lib/lessons/calendar-range.ts`, `lib/lessons/lesson-mappers.ts`              | Batch 6 implementation per audit; pagination if load tests warrant.                                                 |
