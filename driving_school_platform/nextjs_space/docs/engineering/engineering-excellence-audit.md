...
| **Public signup / auth surface** | **Partially hardened:** demo org block (`demo_signup_disabled`); **public signup off by default** (`public_signup_disabled` unless `PUBLIC_SIGNUP_ENABLED=true`); policy in `lib/signup/signup-policy.ts`. **Still open:** invite-only foundation, **real email verification**, **distributed** rate limiting (not in-memory on signup), captcha/Turnstile. Login remains NextAuth credentials; `isEmailVerified` is still a placeholder on signup. See [signup-hardening-plan.md](./signup-hardening-plan.md#current-implementation-status). |
...
| EEA-007 | P2       | Security / abuse | **Partially addressed:** (1) **demo signup blocked** — `demo_signup_disabled`; (2) **public signup disabled by default** — `public_signup_disabled` unless `PUBLIC_SIGNUP_ENABLED=true`; (3) **env explicit opt-in** via `lib/signup/signup-policy.ts`. **Remaining:** invite-only foundation, **email verification**, **distributed rate limit** on signup (not in-memory), **captcha/Turnstile** decision + implementation; `isEmailVerified` still a placeholder. | `app/api/signup/route.ts`, `lib/signup/signup-policy.ts`, [signup-hardening-plan.md](./signup-hardening-plan.md#current-implementation-status) | Next signup-hardening batches per plan; keep production env off until rollout is documented.                        |
...
- **Signup / auth surface:** demo block + public signup **disabled by default** (`PUBLIC_SIGNUP_ENABLED` opt-in). **Still open:** invite-only, **email verification**, distributed **rate limits**, **captcha** — status table in [signup-hardening-plan.md](./signup-hardening-plan.md#current-implementation-status), gaps in [dat-production-readiness-gaps.md](../ops/dat-production-readiness-gaps.md).
