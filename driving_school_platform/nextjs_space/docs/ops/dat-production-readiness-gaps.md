...
- **Demo / tenant isolation (code)** — `Organization.isDemo`, P0 destructive guards, and control-plane tenant guards (settings, feature flags, licensing writes, user create/update) — see [public-demo-policy.md](./public-demo-policy.md); **demo environment, seed/reset, and separated DB** remain the main open operational gap.
...
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Demo vs production data** | Production DB today holds **real** org and user data. A public “try it” link on the **same** deployment lets visitors mutate lessons, users, vehicles, license settings, etc., via existing **SUPER_ADMIN** surfaces. | **Mitigation in code (ongoing):** `Organization.isDemo` plus demo guards on high-risk and control-plane tenant mutations — see [public-demo-policy.md](./public-demo-policy.md) (_Implemented guards_). **Still open:** dedicated demo deployment/DB, demo **seed + reset** cadence, portfolio messaging, deeper smoke matrix, and any follow-ups called out there (e.g. billing webhook / platform onboarding policy). |
...
1. **Demo isolation** — dedicated demo org + seeded low-priv users, separate preview project with reset script, and/or `demo.meengine.io`-style hosting; document access policy ([public-demo-policy.md](./public-demo-policy.md)). P0 API demo guards are **not** a substitute for a separate environment.
