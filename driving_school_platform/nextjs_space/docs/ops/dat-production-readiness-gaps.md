...
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Demo vs production data** | Production DB today holds **real** org and user data. A public “try it” link on the **same** deployment lets visitors mutate lessons, users, vehicles, license settings, etc., via existing **SUPER_ADMIN** surfaces. | **Partial mitigation in code:** `Organization.isDemo` plus P0 demo guards on high-risk mutations (see [public-demo-policy.md](./public-demo-policy.md) — _Implemented guards_). **Still open:** dedicated demo deployment/DB, demo seed + reset cadence, and coverage of remaining admin surfaces (settings, flags, licensing, etc.). |
...
2. **Admin mutation audit** — extend demo (or role) guards to remaining destructive paths (settings, feature flags, licensing, billing admin); inventory org onboarding; decide read-only demo or confirmation modals + audit log.
