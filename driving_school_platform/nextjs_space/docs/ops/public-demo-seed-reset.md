...
**Controlled demo write sandbox:** rows created when `DEMO_WRITE_SANDBOX_ENABLED=true` (see [client-demo-runbook.md](./client-demo-runbook.md#controlled-demo-write-sandbox)) should eventually be removed by the same **scoped demo-org reset** flow once implemented; until then, operators should assume seed plus sandbox creates can fill per-type quotas (theory/driving lessons, theoretical exam, practical exam, vehicle — each max one) without a public reset endpoint.
