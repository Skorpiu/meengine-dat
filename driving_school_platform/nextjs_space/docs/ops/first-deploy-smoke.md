...
5. Open the **deployment URL** from the Vercel UI in a browser (no hostnames recorded here).
6. **`GET /api/health`** — confirm `200` and JSON match expectations in [deployment-readiness.md](./deployment-readiness.md) (DB-free liveness; `Cache-Control: no-store`).
