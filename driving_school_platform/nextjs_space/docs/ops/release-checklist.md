...
| Demo sandbox reset (daily cron)         | `GET /api/cron/demo-sandbox-reset` — see [client-demo-runbook.md](./client-demo-runbook.md#automatic-daily-demo-sandbox-reset)                                                                                                  |
...
| Signup / auth surface status            | [signup-hardening-plan.md](../engineering/signup-hardening-plan.md#current-implementation-status) — phase 1 (demo block + public signup default off) **implemented**; invite / verification / rate limit / captcha **pending**. |
...
- **Before demo (controlled Full Showcase):** run **`pnpm demo:client-ready`** with `DEMO_ORGANIZATION_ID` and the three private persona emails set ([client-demo-runbook.md](./client-demo-runbook.md#client-demo-readiness-smoke)); expect **PASS** before sharing access.
- **Before demo (cron / env):** confirm **Vercel → Settings → Cron Jobs** lists the daily sandbox reset job after deploy; confirm **`CRON_SECRET`** and **`DEMO_ORGANIZATION_ID`** are set in **Vercel → Environment Variables → Production** (no values in git or docs).
- **Before demo (secrets hygiene):** confirm **no** passwords, `CRON_SECRET`, database URLs, or persona credentials appear in README, ops docs, slides, or tickets; share demo login details **privately** only ([client-demo-runbook.md](./client-demo-runbook.md#current-validated-demo-status)).
...
- **Demo sandbox cron (production):** confirm **`CRON_SECRET`** and **`DEMO_ORGANIZATION_ID`** are set in **Vercel → Environment Variables → Production** (no values in git); confirm the cron job is visible under **Settings → Cron Jobs** after deploy; manual **`pnpm demo:sandbox:reset`** remains available ([client-demo-runbook.md](./client-demo-runbook.md#automatic-daily-demo-sandbox-reset)).
- **`PUBLIC_SIGNUP_ENABLED` (production):** should be **intentionally false** — leave **unset** or set **`false`** unless there is a **documented rollout decision** to allow self-serve registration. Only **`true`** (trimmed, case-insensitive) enables `POST /api/signup` for **non-demo** orgs; otherwise clients receive **403** `public_signup_disabled` ([environment-variables.md](./environment-variables.md#public-signup-enabled)). **If `true`:** before deploy, review and document **invite-only policy**, **email verification**, and **distributed rate limiting** (not in-memory on serverless) per [signup-hardening-plan.md](../engineering/signup-hardening-plan.md#current-implementation-status). Demo org signup remains blocked via **`demo_signup_disabled`** regardless of this flag.
