...
- **Controlled demo writes (optional):** if you enable **`DEMO_WRITE_SANDBOX_ENABLED=true`** for a session, confirm it is **intentional** and that operators understand **per-category quotas** (seed rows count; see [client-demo-runbook.md](./client-demo-runbook.md#controlled-demo-write-sandbox)). Turn it off again for default read-mostly posture when the session ends.
