...
- **Before broad public signup:** do **not** enable open self-serve registration (`/auth/register`, `POST /api/signup` for production tenants) until the team has decided and shipped the controls in [signup-hardening-plan.md](../engineering/signup-hardening-plan.md) — at minimum **invite-only or public signup disabled by config**, plus **distributed rate limiting** and **email verification** if signup stays public. Demo org signup remains blocked via `demo_signup_disabled` regardless.
