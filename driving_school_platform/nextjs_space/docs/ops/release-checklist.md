...
| Signup hardening (plan / implementation) | [signup-hardening-plan.md](../engineering/signup-hardening-plan.md) — before broad public self-serve signup, require an explicit product decision on **invite-only**, **email verification**, and **distributed rate limiting** (not in-memory on serverless); implementation batches tracked in that doc. |
...
- **Public signup env (`PUBLIC_SIGNUP_ENABLED`):** confirm production value is **intentional** — unset or `false` disables public signup (**403** `public_signup_disabled`); only **`true`** enables self-serve for non-demo orgs ([environment-variables.md](./environment-variables.md#public-signup-enabled)). If set to **`true`**, document the hardening decision in [signup-hardening-plan.md](../engineering/signup-hardening-plan.md) (invite-only, **distributed rate limiting**, **email verification** still recommended before broad marketing). Demo org signup remains blocked via `demo_signup_disabled` regardless.
