...
| **GitLab CI**                              | Dummy loopback placeholders in YAML `variables`, then **reasserted in `before_script`** before install/check.                                             | `CI=true` / `GITLAB_CI=true` is **not** authorization for a remote database. A remote URL in CI fails the same local rule. Higher-precedence GitLab CI/CD variables can override YAML defaults; runner-provided environment may also supply values to the job shell. The `before_script` shell export deterministically reasserts the loopback `DATABASE_URL` / `DIRECT_URL` after environment injection. |
...
| **Disposable integration tests (DEC-070)** | Canonical constructed local URL (`127.0.0.1:55432` / `dat_it`) or CI `INTEGRATION_DATABASE_URL` (`dat-integration-postgres:5432` / `dat_it`).             | Purpose-scoped mutation authority only. Validated before any write. Application `DATABASE_URL` is not authority. `CI` / `GITLAB_CI` / `NODE_ENV` / `VERCEL` do not expand the allowlist. Ordinary `pnpm check` does not use this database. See [database-integration-tests.md](./database-integration-tests.md).                                                                                          |
...
The disposable integration harness (`pnpm test:integration`, DEC-070) is a **third** purpose-scoped path. It does not load application or operator env files. It overwrites `DATABASE_URL` / `DIRECT_URL` in a child process after validating the disposable identity. It is not ordinary development and not remote migration.
...
| Kind                                                | Where                                                                                                                                                                                                                                                 |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Dummy / placeholder values for `env-check` + Prisma | Top-level `variables` in `.gitlab-ci.yml` (`DATABASE_URL`, `DIRECT_URL`, `NEXTAUTH_SECRET`) **and** `before_script` shell `export` of loopback `DATABASE_URL` / `DIRECT_URL` before `pnpm install` / `pnpm check` (`gitlab-ci-database-isolation-v1`) |

CI uses loopback placeholder URLs so `prisma generate` and builds can run without a live database for unit tests. Those placeholders pass the local isolation rule because they are loopback. Setting `CI=true` does **not** authorize a remote `DATABASE_URL`. Higher-precedence GitLab CI/CD variables can override YAML defaults; runner-provided environment may also supply values to the job shell. The `before_script` shell export deterministically reasserts the loopback `DATABASE_URL` / `DIRECT_URL` after environment injection. This does not connect to a database and does not run migrations.

The separate `database-integration` job (DEC-070) does **not** attach Postgres to `check`. It uses GitLab `services:` plus `INTEGRATION_DATABASE_URL` and an explicit `--provision=ci-external` script. `CI=true` remains non-authority. See [database-integration-tests.md](./database-integration-tests.md).
...
| `CI`                                                            | Set by GitLab; consumed by Playwright config. **Not** integration-database authorization (DEC-070).                                                                                                                                                                                                                                                                                                                                               |
| `DAT_INTEGRATION_PROVISION_MODE`                                | Optional consistency env for the disposable integration harness. When set, must match explicit `--provision` (`local-compose` or `ci-external`). Does not expand the database allowlist. See [database-integration-tests.md](./database-integration-tests.md).                                                                                                                                                                                    |
| `INTEGRATION_DATABASE_URL`                                      | CI external integration mode only. Must match host `dat-integration-postgres`, port `5432`, database/user/password `dat_it`. Local Compose mode ignores this variable. Not a Production secret.                                                                                                                                                                                                                                                   |
| `E2E_BASE_URL`                                                  | Preferred Playwright tenant origin for optional demo/production smoke ([e2e-smoke.md](./e2e-smoke.md)); e.g. `https://demo.meengine.io`. Falls back to `PLAYWRIGHT_BASE_URL`, then `http://localhost:3000`. Used by `playwright.smoke.config.ts`, not by local disposable E2E.                                                                                                                                                                    |
...
| `E2E_SKIP_WEB_SERVER`                                           | When `1`, smoke/mobile Playwright configs do not auto-start `pnpm dev` (use with a running local server or remote `E2E_BASE_URL`). Not used by the local disposable E2E orchestrator.                                                                                                                                                                                                                                                             |
| `PLAYWRIGHT_BASE_URL`, `BASE_URL`                               | Legacy Playwright base URL aliases for **smoke** (`playwright.smoke.config.ts`). Local disposable E2E ignores these and uses `http://127.0.0.1:13000`.                                                                                                                                                                                                                                                                                            |
| `DAT_E2E_ORCHESTRATOR_ACTIVE`                                   | Injected by `pnpm test:e2e`. Must be `1` for `playwright.config.ts` and local specs. Not an operator-set Production flag.                                                                                                                                                                                                                                                                                                                         |
| `DAT_E2E_BASE_URL`                                              | Injected by `pnpm test:e2e`. Must be the exact local app URL `http://127.0.0.1:13000`.                                                                                                                                                                                                                                                                                                                                                            |
...
| `CRON_SECRET`                                                   | Optional locally. **Required in Vercel Production** for protected cron routes such as the daily demo sandbox reset (`GET /api/cron/demo-sandbox-reset`) and rate-limit bucket cleanup (`GET /api/cron/rate-limit-cleanup`). Vercel sends `Authorization: Bearer <CRON_SECRET>` on cron invocations. Never commit a real value.                                                                                                                    |
...
### `CRON_SECRET` and `DEMO_ORGANIZATION_ID` (cron / operator scripts)

- **`CRON_SECRET`** — long random value in **Vercel Production** only (for typical hosted demo / protected cron routes). Used by Vercel Cron to authorize `GET /api/cron/demo-sandbox-reset` and `GET /api/cron/rate-limit-cleanup`. If missing at runtime, those routes return **503** with a safe JSON error (no stack traces).
...
- **Schedules:** `0 3 * * *` for demo sandbox reset and `30 3 * * *` for rate-limit bucket cleanup in `vercel.json`; Hobby plans may run within the hour window.
