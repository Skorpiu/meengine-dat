...
| `EMAIL_PROVIDER`                                                           | Optional. Read only by `lib/email/email-service.ts` (not `lib/env.ts` / `env-check`). Unset, empty, or `noop` → **noop** (no network, no delivery). `resend` / `postmark` / `smtp` are recognized but **not implemented** — `sendEmail()` returns a controlled error without sending. Unknown values are rejected safely. No real email is sent in production until a later batch adds a provider adapter and optional send flags. See [email-provider-evaluation.md](../engineering/email-provider-evaluation.md). |
