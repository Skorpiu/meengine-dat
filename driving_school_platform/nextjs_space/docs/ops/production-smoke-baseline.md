...
| **Tenant admin (authenticated)** | `/admin` (Schedule Map), `/admin/users` (People: Students, Instructors, Onboarding), `/admin/lessons`, `/admin/vehicles`, `/admin/license` (**Plan** — read-only), `/profile` reachable for authorized tenant admin flows under test. **Settings** (`/admin/settings`) is operator/internal — **not** in school admin navbar (DEC-026); direct URL smoke optional for operators only. |
...
- [production-readiness-cutline.md](../../../../docs/architecture/production-readiness-cutline.md) — controlled first B2B client cutline (DEC-032).
