...
const getUserPreferencesMock = getUserPreferences as unknown as ReturnType<typeof vi.fn>;
const updateUserPreferencesMock = updateUserPreferences as unknown as ReturnType<typeof vi.fn>;
const logConfigurationChangeMock = logConfigurationChange as unknown as ReturnType<typeof vi.fn>;
...
describe('/api/user/preferences (tenant scoping)', () => {
  it('GET returns 400 when session has no organizationId', async () => {
    getServerSessionMock.mockResolvedValue({ user: { id: 'u1', role: 'STUDENT', organizationId: null } });
    resolveTenantOrganizationIdMock.mockResolvedValue({ host: 'www.meengine.io', organizationId: null });

    const res = await GET(new Request('http://localhost/api/user/preferences') as any);
    expect(res.status).toBe(400);
    expect(getUserPreferencesMock).not.toHaveBeenCalled();
  });

  it('GET returns 403 when tenant org != session org', async () => {
    getServerSessionMock.mockResolvedValue({ user: { id: 'u1', role: 'STUDENT', organizationId: 'orgA' } });
  ...
  it('PUT updates preferences when tenant matches', async () => {
    ...
    getUserPreferencesMock.mockResolvedValue({ id: 'pref1', theme: 'dark' });
    updateUserPreferencesMock.mockResolvedValue({ id: 'pref1', theme: 'light' });
