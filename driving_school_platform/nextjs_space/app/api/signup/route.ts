...
export async function POST(request: NextRequest) {
  try {
    ...
    const transmissionType =
      typeof body.transmissionType === "string" ? body.transmissionType : null;

    const instructorLicenseNumber =
      typeof body.instructorLicenseNumber === "string"
        ? body.instructorLicenseNumber
        : null;
    ...
    const isLocal =
      !!host && (LOCAL_HOSTS.has(host) || host.endsWith(".localhost"));

    const effectiveOrganizationId =
      tenant.organizationId ?? (isLocal ? organizationId : null);
      ...
      async (tx: Prisma.TransactionClient) => {
        ...
        const user = await tx.user.create({
          data: {
            ...
            city,
            postalCode,
            // Email verification is not implemented yet; this is tracked as production hardening.
          ...
          if (transmissionType) {
            ...
            if (!transmission) {
              transmission = await tx.transmissionType.findFirst({
                where: {
                  code:
                    transmissionType.toUpperCase() === "MANUAL" ? "MT" : "AT",
                },
              });
            }
          }
