...
type VehicleCategoryRef = {
  name: string;
};

type VehicleTransmissionTypeRef = {
  name: string;
};
...
type VehicleRow = {
  ...
  hasDualControls?: boolean | null;
  hasDashcam?: boolean | null;
  category?: VehicleCategoryRef | null;
  transmissionType?: VehicleTransmissionTypeRef | null;
};
...
export function VehiclesManagementClient() {
  ...
  const [vehicleToDelete, setVehicleToDelete] = useState<VehicleRow | null>(
    null,
  );
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      inUse: vehicles.filter((v: VehicleRow) => v.status === "IN_USE").length,
      maintenance: vehicles.filter((v: VehicleRow) => v.underMaintenance)
        .length,
    };
  }, [vehicles]);
  ...
  const handleMaintenanceToggle = useCallback(
    async (vehicleId: number, currentStatus: boolean) => {
      try {
        ...
        showError(
          error instanceof Error
            ? error
            : new Error("Failed to update maintenance status"),
        );
      }
    },
    [fetchVehicles],
   ...
   */
  const handleEditVehicle = useCallback((vehicle: VehicleRow) => {
   ...
   */
  const handleDeleteClick = useCallback((vehicle: VehicleRow) => {
  ...
  const filteredVehicles = useMemo(() => {
    ...
    return vehicles.filter(
      (v: VehicleRow) => v.category && categories.includes(v.category.name),
    );
  ...
  const filteredStats = useMemo(() => {
    ...
    return {
      ...
      available: filteredVehicles.filter(
        (v: VehicleRow) =>
          v.isActive && v.status === "AVAILABLE" && !v.underMaintenance,
      ).length,
      inUse: filteredVehicles.filter((v: VehicleRow) => v.status === "IN_USE")
        .length,
      maintenance: filteredVehicles.filter(
        (v: VehicleRow) => v.underMaintenance,
      ).length,
    };
  ...
  const getVehicleIcon = (vehicle: VehicleRow) => {
    ...
    <>
      ...
      <div className="mb-6">
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            variant={
              selectedFleet === "heavy_passenger" ? "default" : "outline"
            }
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>
                {selectedFleet === "all"
                  ? "Fleet Overview"
                  : selectedFleet === "motorcycle"
                    ? "Motorcycles"
                    : selectedFleet === "car"
                      ? "Cars"
                      : selectedFleet === "heavy_goods"
                        ? "Heavy Goods Vehicles"
                        : "Heavy Passenger Vehicles"}
              </CardTitle>
              <CardDescription>
                {filteredVehicles.length} vehicle
                {filteredVehicles.length !== 1 ? "s" : ""} in this category
              </CardDescription>
            </div>
